...
export class LikedLineItem extends vscode.TreeItem {
    constructor(
        ...
        super(data.label || `${vscode.Uri.file(data.filePath).with({ scheme: 'untitled' }).fsPath.split('/').pop()}:${data.lineNumber + 1}`, collapsibleState);
        ...
        this.description = `Line ${data.lineNumber + 1} - ${data.lineContent.trim().substring(0, 70)}${data.lineContent.trim().length > 70 ? '...' : ''}`;

        // Icon for the TreeItem
        this.iconPath = new vscode.ThemeIcon('heart'); // Use a heart icon
